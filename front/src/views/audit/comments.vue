  ...
  <q-card>
    <q-card-section>
      ...
      <vxe-table ref="tableRef" :data="tableData" :loading="loading" stripe border>
        ...
        <!-- 评论时间列 -->
        <vxe-column field="CommentDate" title="评论时间" width="180" sortable>
          <template #default="{ row }"> {{ formatDate(row.CommentDate) }} </template>
        </vxe-column>
        <!-- 邮箱列 -->
        <vxe-column field="Email" title="邮箱" width="200"></vxe-column>
        <!-- 操作系统列 -->
        <vxe-column field="OperatingSystem" title="操作系统" width="160"></vxe-column>
        <!-- 浏览器列 -->
        ...
        <vxe-column title="操作" width="120" fixed="right">
          <template #default="{ row }">
            <q-btn-group push>
                ...
                <q-popup-proxy transition-show="scale" transition-hide="scale">
                  <q-card>
                    ...
                    <q-card-actions align="right">
                      <q-btn flat label="确认" color="negative" v-close-popup @click="deleteComment(row)" />
              ...
              <!-- 通过审核按钮 -->
              <q-btn size="md" color="positive" icon="check" dense @click="passComment(row)">
                <q-tooltip>通过审核</q-tooltip>
              </q-btn>
              <!-- 查看详情按钮 -->
              <q-btn size="md" color="info" icon="visibility" dense :href="`/${row.PostId}?cid=${row.Id}#comment`" target="_blank">
...
const loadPageData = async () => {
  loading.value = true
  try {
    const response = await api.post(`/comment/GetPendingComments?page=${pagination.value.page}&size=${pagination.value.rowsPerPage}`) as ApiResponse
...
const deleteComment = async (row: Comment) => {
  try {
    const response = await api.post(`/comment/delete/${row.Id}`) as ApiResponse
    if (response?.Success) {
...
// 通过审核
const passComment = async (row: Comment) => {
  const response = await api.post('/comment/pass/' + row.Id) as ApiResponse
  if (response?.Success) {
