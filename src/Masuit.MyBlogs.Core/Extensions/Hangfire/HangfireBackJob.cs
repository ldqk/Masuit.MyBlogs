...
public sealed class HangfireBackJob : Disposable, IHangfireBackJob
{
    ...
    public void RecordPostVisit(int pid, string ip, string refer, string url)
    {
        ...
        if (count > 0)
        {
            ...
            if (double.IsInfinity(post.AverageViewCount))
            {
    ...
    public void EverydayJob()
    {
        ...
        postService.GetQuery(p => p.ExpireAt < DateTime.Now && p.Status == Status.Published).ExecuteUpdate(s => s.SetProperty(p => p.Status, Status.Takedown).SetProperty(p => p.IsFixedTop, false));
    }
    ...
    public void UpdateLinkWeight(string referer, string ip)
    {
        ...
        var list = linksService.GetQuery(l => referer.Contains(l.UrlBase)).ToListWithNoLock();
        ...
        var time = DateTime.Now.AddMonths(-1);
        loopbackService.GetQuery(b => b.Time < time).ExecuteDelete();
        linksService.SaveChanges();
    }
    ...
    public void CreateLuceneIndex()
    {
        ...
        var postService = _serviceScope.ServiceProvider.GetRequiredService<IPostService>();
        var posts = postService.GetQuery(p => p.Status == Status.Published && p.LimitMode != RegionLimitMode.OnlyForSearchEngine);
        searchEngine.LuceneIndexer.DeleteAll();
        var count = posts.Count();
        var pages = count * 1m / 100;
        for (int i = 0; i < pages; i++)
        {
            searchEngine.LuceneIndexer.CreateIndex(posts.Skip(i * 100).Take(100).ToList(), false);
        }
    }
