myApp.controller("partner", ["$scope", "$http", "$timeout", "NgTableParams", function ($scope, $http, $timeout, NgTableParams) {
	...
	xmSelect.render({
		...
		data:[
				{name:"默认",value:0,selected:$scope.orderby==0},
				{name:"竞价",value:1,selected:$scope.orderby==1},
				{name:"曝光量",value:2,selected:$scope.orderby==2},
				{name:"点击量",value:3,selected:$scope.orderby==3},
				{name:"日均点击",value:4,selected:$scope.orderby==4},
				{name:"点击率",value:5,selected:$scope.orderby==5},
			],
		on: function (data) {
	...
	this.GetPageData = function (page, size) {
		$http.get(`/partner/getpagedata?page=${page}&size=${size}&kw=${$scope.kw}&orderby=${$scope.orderby||0}`).then(function (res) {
