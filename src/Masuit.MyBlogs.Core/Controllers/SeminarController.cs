...
public sealed class SeminarController : BaseController
{
    ...
    public async Task<ActionResult> Index(int id, [Optional] OrderBy? orderBy, [Range(1, int.MaxValue, ErrorMessage = "页码必须大于0")] int page = 1, [Range(1, 50, ErrorMessage = "页大小必须在0到50之间")] int size = 20)
    {
        ...
        {
            OrderBy.Trending => await PostService.GetQuery(PostBaseWhere().And(p => p.Seminar.Any(x => x.Id == id))).OrderByDescending(p => p.PostVisitRecordStats.Where(e => e.Date >= h24).Sum(e => e.Count)).ProjectDto().ToPagedListAsync(page, size),
            _ => await PostService.GetQuery(PostBaseWhere().And(p => p.Seminar.Any(x => x.Id == id))).OrderBy($"{nameof(Post.IsFixedTop)} desc,{(orderBy ?? OrderBy.ModifyDate).GetDisplay()} desc").ProjectDto().ToPagedListAsync(page, size)
        };
        ViewBag.Id = s.Id;
    ...
    public async Task<ActionResult> Get(int id)
    {
        ...
        return ResultData(seminar.ToDto());
    }
    ...
    {
        var list = SeminarService.GetAll(s => s.Title).ProjectDto().ToPooledListScope();
