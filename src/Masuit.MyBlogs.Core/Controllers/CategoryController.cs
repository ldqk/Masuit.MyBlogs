...
public sealed class CategoryController : BaseController
{
    ...
    {
        var categories = CategoryService.GetQuery(c => c.Status == Status.Available, c => c.Name).ProjectCommand().ToListWithNoLock();
        var list = categories.ToDto();
